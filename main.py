...
def time_str(t=utime.localtime()):
                ...
                load_html("static/index.html"),
                {
                    ...
                    "moon_age": (
                        datetime_diff_seconds(
                            utime.localtime(), known_moon_phase_and_time[1]
                        )
                        / seconds_in_day
                    )
                    % moon_cycle_length_days,
...
def save_settings(cl: socket.socket, parameters: dict):
    ...
    brightness_night_mode = int(
        parameters.get("nightModeBrightness", brightness_night_mode)
    )
    if brightness_night_mode < 0:
        brightness_night_mode = 0
    if brightness_night_mode > 255:
        brightness_night_mode = 255
    ...
    is_color_random = (
        parameters.get("isColorRandom", "true" if is_color_random else "false")
        == "true"
    )

    is_night_mode = (
        parameters.get("isNightMode", "true" if is_night_mode else "false") == "true"
    )
    ...
    night_mode_start = parse_hour_and_minute(
        parameters.get("nightModeStart", get_hour_and_minute(night_mode_start))
    )

    night_mode_end = parse_hour_and_minute(
        parameters.get("nightModeEnd", get_hour_and_minute(night_mode_end))
    )

    central_url = parameters.get("central_url", central_url)
...
def loading_animation(phase: int):
    ...
    for i in range(
        moon_phases.index(phase), len(moon_phases) + moon_phases.index(phase)
    ):
        led_strip.set_pixel(
            moon_phases[i % len(moon_phases)], led_color, led_brightness
        )
        time.sleep(0.1)
...
def current_moon_phase_id():
    ...
    moon_age = (
        datetime_diff_seconds(utime.localtime(), known_moon_phase_and_time[1])
        / seconds_in_day
    ) % moon_cycle_length_days
